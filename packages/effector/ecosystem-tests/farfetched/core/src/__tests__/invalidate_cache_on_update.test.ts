...
import { withFactory } from "@virentia/effector";
    ...
    const query = withFactory({
      fn: () =>
        createQuery({
          handler: vi
            .fn()
            .mockImplementationOnce(() => setTimeout(100).then(() => "old from remote source"))
            .mockImplementationOnce(() => setTimeout(100).then(() => "new from remote source")),
        }),
      sid: "q",
    });
    ...
    const query = withFactory({
      fn: () =>
        createQuery({
          handler: vi
            .fn()
            .mockImplementationOnce((p: string) => setTimeout(100).then(() => `old result: ${p}`))
            .mockImplementationOnce((p: string) => setTimeout(100).then(() => `new result: ${p}`))
