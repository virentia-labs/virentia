...
import { broadcast as broadcastIndex } from "effector-storage";
import { broadcast, persist } from "effector-storage/broadcast";
import { log } from "effector-storage/log";
import { either } from "effector-storage/tools";
...
it("should be nil adapter in unsupported environment", async () => {
  ...
  try {
    ...
    expect(logger.mock.calls[0]).toEqual(['[log adapter] get value for key "unsupported"']);
  } finally {
    global.BroadcastChannel = _BroadcastChannel;
  }
});
