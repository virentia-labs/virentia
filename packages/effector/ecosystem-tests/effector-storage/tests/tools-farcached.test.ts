...
import { inMemoryCache, localStorageCache, sessionStorageCache } from "@farfetched/core";
import { allSettled, createEffect, createEvent, createStore, fork, sample } from "effector";
import { afterAll, beforeAll, beforeEach, expect, it, vi } from "vitest";
import { farcached, persist } from "effector-storage";
...
it("store new value should be saved to storage", async () => {
  ...
  expect(global.localStorage.getItem("counter2")).toBe(`{"value":22,"timestamp":${NOW}}`);
});
...
it("should be possible to inject different cache adapter (no initial value)", async () => {
  ...
  expect(global.localStorage.getItem("counter4")).toBe('{"value":2,"timestamp":1000000001000}');

  // sessionStorage should contain value from scopeB
  expect(global.sessionStorage.getItem("counter4")).toBe('{"value":3,"timestamp":1000000002000}');
});
