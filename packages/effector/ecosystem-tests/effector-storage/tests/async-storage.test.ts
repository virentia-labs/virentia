...
import { asyncStorage as asyncStorageIndex } from "effector-storage";
...
it("different storage instances should not interfere", async () => {
  ...
  expect(await mockStorage2.getItem("custom")).toBe("222");
  expect($counter1.getState()).toBe(JSON.parse((await mockStorage1.getItem("custom")) as any));
  ($counter2 as any).setState(444);
  await timeout(0);
  expect(await mockStorage1.getItem("custom")).toBe("333");
  expect(await mockStorage2.getItem("custom")).toBe("444");
  expect($counter2.getState()).toBe(JSON.parse((await mockStorage2.getItem("custom")) as any));
});
...
it("should not throw on storage access error", async () => {
  ...
  expect(() => persist({ store: $storex, adapter: throwAdapter, fail })).not.toThrow();
