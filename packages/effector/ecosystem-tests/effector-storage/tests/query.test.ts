...
import { query as queryIndex } from "effector-storage";
import {
  locationAssign,
  locationReplace,
  persist,
...
it("should preserve url hash part", () => {
  global.history = createHistoryMock(null, "", "http://domain.test?id=8888#test");
...
it("should preserve state by default", () => {
  ...
  expect(mockPushState.mock.calls[0]).toEqual([{ test: "test" }, "", "/?id=111"]);
});
