...
import { createPersist, persist } from "effector-storage";
...
it("should be ok on good parameters", () => {
  ...
  expect(() => persist({ adapter: dumbAdapter, store: $store0, key: "_store0_" })).not.toThrow();
  expect(() =>
    persist({
      adapter: dumbAdapter,
      source: $store1,
  ...
  expect(() => persist({ adapter: dumbAdapter, store: $store0named })).not.toThrow();
  expect(() =>
    persist({
      adapter: dumbAdapter,
      source: $store1named,
...
it("should handle wrong parameters", () => {
  ...
  expect(() => persist({ adapter: dumbAdapter } as any)).toThrow(/Store or source is not defined/);
  ...
  expect(() => persist({ adapter: dumbAdapter, source: event, target: $store })).toThrow(
    /Key or name is not defined/,
  );
  expect(() =>
    persist({
