...
import { log, persist } from "effector-storage";
import { log as logNested } from "effector-storage/log";
...
it("store should ignore initial `undefined` from storage value", () => {
  ...
  expect(logger.mock.calls[0]).toEqual(['[log adapter] get value for key "log::counter0"']);
});
...
it("store new value should be ignored by storage", () => {
  ...
  expect(logger.mock.calls[1]).toEqual(['[log adapter] set value "42" with key "log::counter1"']);
});
...
it("should use console.log by default", () => {
  ...
  expect(logger).toHaveBeenCalledTimes(1);
  expect(logger.mock.calls[0]).toEqual(['[log adapter] get value for key "log::counter3"']);
});
