...
import { storage as storageIndex } from "effector-storage";
...
it("store new value should be saved to storage", () => {
  ...
  expect($counter2.getState()).toBe(JSON.parse(mockStorage.getItem("counter2") as any));
});
...
it("different storage instances should not interfere", () => {
  ...
  expect(mockStorage2.getItem("custom")).toBe("222");
  expect($counter1.getState()).toBe(JSON.parse(mockStorage1.getItem("custom") as any));
  ($counter2 as any).setState(444);
  expect(mockStorage1.getItem("custom")).toBe("333");
  expect(mockStorage2.getItem("custom")).toBe("444");
  expect($counter2.getState()).toBe(JSON.parse(mockStorage2.getItem("custom") as any));
});
