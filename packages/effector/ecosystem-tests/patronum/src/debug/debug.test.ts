...
import { createEvent, createEffect, createStore, createDomain, sample } from "effector";
...
test("debug domain", async () => {
  ...
  const effect = domain.createEffect().use((payload) => Promise.resolve("result" + payload));
