...
describe("Form tests", () => {
  ...
  test("submitted and validated", async () => {
    ...
    const mockedValidatedAndSubmittedFx = createEffect(mockedValidatedAndSubmittedFn);
  ...
  test('"validated and submitted" event doesn\'t called after "validated" without "submitted"', async () => {
    ...
    const form = createForm({
      schema: { a: "", b: "", c: "", d: "", e: "" },
      validation: (values) => (values.a.length > 4 ? null : { a: "some error" }),
    });
  ...
  describe("snapshots", () => {
    test("snapshot updated after submit & validate", async () => {
      ...
      expect(scope.getState(form.$values)).not.toStrictEqual(scope.getState(form.$snapshot));

      await allSettled(form.submit, { scope });

      expect(scope.getState(form.$values)).toStrictEqual(scope.getState(form.$snapshot));
    });
