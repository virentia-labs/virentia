...
// Virentia upstream partial skip reason: Базовая serialize/fork(values)/serialize:ignore совместимость включена; custom serialize, domain serialization, onlyChanges:false и SID diagnostics пока расходятся с Effector.
describe("upstream fork/serialize.test.ts", () => {
  ...
  describe("onlyChanges: false", () => {
    it("avoid serializing combined stores when they are not changed", async () => {
      const app = createDomain();
      const newMessage = app.createEvent();
      const messages = app.createStore(0, { sid: "messages" }).on(newMessage, (x) => x + 1);
      const stats = combine({ messages });
      const scope = fork(app);
  ...
  describe("serialize: missing sids", () => {
    ...
    // Virentia upstream skip reason: exact missing-sid diagnostics are intentionally not matched yet.
    test.skip("serialize: warns about missing sids", () => {
      // forcing missing sid
  ...
  `);
    ...
    // Virentia upstream skip reason: duplicate SID diagnostics need a separate SID registry policy.
    test.skip("serialize: throws if duplicated sids", () => {
    ...
    test("serialize: doesn not warn on mapped or combined stores", () => {
      ...
      const result = serialize(scope);
      expect($combine.sid ?? null).toEqual(null);
      expect(scope.getState($store)).toEqual("scope value");
