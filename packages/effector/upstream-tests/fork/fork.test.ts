...
// Virentia upstream partial skip reason: Простые fork/getState/tuple handlers включены; domain scopes, SID-map handlers, exact validations и scope watch ordering пока расходятся с Effector.
describe("upstream fork/fork.test.ts", () => {
  ...
  describe("fork values support", () => {
    ...
    test("values initialization consistency", async () => {
       ...
       * */
      let sumStore: Store<[number, any[]]> = createStore([0, []] as [number, any[]], {
        sid: "sum",
      });
      ...
      function fab(n: number) {
        ...
        sumStore = combine(sumStore, store, (prevVal, x) => [x, prevVal] as [number, any[]]);
        const secondOne = createStore(n, { sid: `${n}.2` });
        const third = store.map((x) => x + 1);
        const both = combine(third, secondOne, (x, y) => x * y);
        sumStore = combine(sumStore, both, (prevVal, x) => [x, prevVal] as [number, any[]]);
      ...
      expect(scope.getState(finalStore)).toEqual([
        { store: 0, secondOne: 0, third: 1, both: 0 },
        { store: 1, secondOne: 0, third: 2, both: 0 },
        { store: 2, secondOne: 2, third: 3, both: 6 },
        { store: 0, secondOne: 3, third: 1, both: 3 },
        { store: 4, secondOne: 0, third: 5, both: 0 },
        { store: 5, secondOne: 5, third: 6, both: 30 },
        { store: 0, secondOne: 6, third: 1, both: 6 },
        { store: 7, secondOne: 0, third: 8, both: 0 },
        { store: 8, secondOne: 8, third: 9, both: 72 },
        { store: 0, secondOne: 9, third: 1, both: 9 },
      ]);
      expect(basicCase).toEqual({
        "0.1": 0,
        "1.2": 0,
        "3.1": 0,
        "4.2": 0,
      ...
      expect(scope.getState(sumStore)).toEqual([
        9,
        [
          0,
          [
            ...
            [
              ...
              [
                0,
                [7, [6, [0, [30, [5, [0, [4, [3, [0, [6, [2, [0, [1, [0, [0, [0, []]]]]]]]]]]]]]]]],
              ],
            ],
          ],
        ],
  ...
  describe("fork handlers support", () => {
    test("handlers as js Map", async () => {
      ...
      const acc = createStore<string[]>([]).on(fx.doneData, (list, val) => [...list, val]);
  ...
  describe("handlers validation", () => {
    ...
    test("passing non-effect unit to handlers should throw", () => {
      ...
      }).toThrowErrorMatchingInlineSnapshot(`"Handlers map can contain only effects as keys"`);
    });
  ...
  test("edge case", async () => {
    ...
    const $isFeatureEnabled = createStore(false, { sid: "feature" }).on(enableFeature, () => true);

    const $isWidgetEnabled = combine([$isCountryPage, $isFeatureEnabled], (s) => s.every(Boolean));
  ...
  describe("scope watch calls", () => {
    test("without values", async () => {
      const aWatchFn = vi.fn();
      const aUpdWatchFn = vi.fn();
      const mappedWatchFn = vi.fn();
      const mappedUpdWatchFn = vi.fn();
      const combinedWatchFn = vi.fn();
      const combinedUpdWatchFn = vi.fn();
    ...
    function addWatch(store: Store<any>, fnA: (value: any) => any, fnB: (value: any) => any) {
      let inited = false;
      store.watch((upd) => {
        if (!inited) {
          inited = true;
