...
// Virentia upstream skip reason: Большой набор fork/domain/serialize edge cases, завязанный на внутреннюю модель Effector.
describe.skip("upstream fork/index.test.ts", () => {
  ...
  describe("imperative call support", () => {
    ...
    describe("support imperative effect calls in effects", () => {
      ...
      test("scope isolation", async () => {
        ...
        const words = createStore<string[]>([]).on([addWord.doneData, pushWord], (list, word) => [
          ...list,
          word,
        ]);
    ...
    `);
      ...
      test("concurrency", async () => {
        ...
        const scopeC = fork();
        await Promise.all([
          allSettled(start, { scope: scopeA }).then(() => allSettled(start, { scope: scopeC })),
          allSettled(start, { scope: scopeA }).then(() => allSettled(start, { scope: scopeB })),
        ]);
        await Promise.all([
          allSettled(start, { scope: scopeA }),
          allSettled(start, { scope: scopeB }).then(() => allSettled(start, { scope: scopeC })),
        ]);
        await Promise.all([
    ...
    `);
      ...
      test("concurrency 3", async () => {
        ...
        await Promise.all([
          allSettled(fx, { scope: scopeA }).then(() => allSettled(fx, { scope: scopeC })),
          allSettled(fx, { scope: scopeA }).then(() => allSettled(fx, { scope: scopeB })),
        ]);
        await Promise.all([
          allSettled(fx, { scope: scopeA }),
          allSettled(fx, { scope: scopeB }).then(() => allSettled(fx, { scope: scopeC })),
        ]);
        await Promise.all([
          allSettled(fx, { scope: scopeB }),
          allSettled(fx, { scope: scopeA }),
  ...
  describe("getState with same sids", () => {
    test("reading values from stores with same sid is correct", async () => {
      ...
      const $directValue = createStore(false, { sid: "sameSid" }).on(touchedDirect, (v) => !v);
  ...
  describe("diamond deps (issue #613)", () => {
    test("with getState (should pass)", async () => {
      ...
      const $history = createStore<{ path: string; search: string } | null>(null).on(
        historyChanged,
        (_, upd) => upd,
      );
