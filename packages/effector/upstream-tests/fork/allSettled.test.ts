...
// Virentia upstream partial skip reason: allSettled return value включён; domain-based allSettled, pending scope drain, transactions snapshots и exact validation diagnostics пока расходятся с Effector.
describe("upstream fork/allSettled.test.ts", () => {
  test("allSettled first argument validation", async () => {
    await expect(
      //@ts-expect-error
      allSettled(null, { scope: fork() }),
    ).rejects.toThrowErrorMatchingInlineSnapshot(`"first argument should be unit"`);
    ...
    await expect(allSettled(createEvent(), { scope: fork() })).resolves.toBeUndefined();
  ...
  test("derived units are not accepted", async () => {
    ...
    ).rejects.toThrow("unit should be targetable");
  ...
  test("allSettled(scope)", async () => {
    ...
    expect(Object.values(serialize(scope))).toEqual([true]);
  });
  ...
  describe("allSettled return value", () => {
    ...
    describe("attach support", () => {
      test("throw in original effect", async () => {
        const original = createEffect<void, any>(async () => Promise.reject("err"));
        const fx = attach({
          effect: original,
          mapParams: (_: void) => {},
        });
  ...
  describe("transactions", () => {
    ...
    // Virentia upstream skip reason: awaits nested effects inside effect handlers; this needs reentrant async effect flush in core.
    test.skip("", async () => {
      ...
      const words = createStore<string[]>([]).on(inner2.done, (list, { params: word }) => [
        ...list,
        word,
      ]);
