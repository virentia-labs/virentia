...
// Virentia upstream skip reason: Effector inspect/inspectGraph protocol и декларации графа не реализованы в Virentia.
describe.skip("upstream inspect.test.ts", () => {
  ...
  `);
    ...
    test("should be possible to track chain of events in specific scope", () => {
      ...
      expect(argumentHistory(trackMock).join(",").includes("SHOULD_NOT_BE_TRACKED")).toBe(false);
      expect(argumentHistory(trackMock).join(",").includes("MUST_BE_TRACKED")).toBe(true);
      expect(argumentHistory(trackMock)).toMatchInlineSnapshot(`
    Array [
  ...
  function compactDeclaration(d: Declaration) {
    ...
    if (d.type === "factory")
      return `factory, ${d.method}, ${d.sid}, ${d.name}, parent: ${typeof d.region}`;
  ...
  `);
    ...
    describe("region support", () => {
      test("one-level withRegion", () => {
        ...
        const nonRegionalUnitDeclared = vi.fn();
        const regionalUnitDeclared = vi.fn();
        inspectGraph({
          fn: (d) => {
      ...
      inspectGraph({
        fn: (d) => {
          if (d.loc) {
            const file = d.loc.file.split("/").at(-1) || "";
            const units = unitsByFile[file] || [];
            const name = d.region ? `${d.region.region?.meta.name!}/${d.name!}` : d.name!;
