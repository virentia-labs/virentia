...
// Virentia upstream skip reason: Закрепляет Effector nested trigger priority; Virentia scheduler не обещает такой eager recursion порядок.
it.skip("safe with nested triggers", () => {
  const a = createEvent();
  const b = createEvent();
  const c = createEvent();
...
test("store.reset(event)", () => {
  ...
  const listSize = createStore(3)
    .on(inc, (n) => n + 1)
    .reset(reset);
  const currentList = createStore(Array.from({ length: listSize.getState() }, (_, n) => n))
...
// Virentia upstream skip reason: Проверяет точные Effector no-dull intermediate update snapshots; Virentia lazy graph dedupe/recompute sequence отличается.
test.skip("no dull updates", () => {
...
// Virentia upstream skip reason: Проверяет точную Effector no-dull последовательность для fork/map; scoped final state покрывается fork tests.
test.skip("no dull updates with fork and map", async () => {
  const store = createStore(false);
  const e1 = createEvent<boolean>();
  const e2 = createEvent<boolean>();
  const fn1 = vi.fn();
...
// Virentia upstream skip reason: Проверяет точную Effector no-dull последовательность для fork/combine; scoped final state покрывается fork/combine behavior tests.
test.skip("no dull updates with fork and combine", async () => {
  const store = createStore(false);
  const e1 = createEvent<boolean>();
  const e2 = createEvent<boolean>();
  const fn1 = vi.fn();
...
test("smoke", async () => {
  const used = vi.fn((x) => Promise.resolve(x));
  const usedDone = vi.fn((x) => Promise.resolve(x));
