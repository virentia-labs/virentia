...
describe("upstream region.test.ts", () => {
  muteErrors("forward");
...
`);
  ...
  // Virentia upstream skip reason: Проверяет точное количество eager вызовов `.map` внутри region; Virentia lazy model сохраняет lifetime cleanup, но допускает другие промежуточные вычисления.
  it.skip("binds units to region lifetime", () => {
    const fn = vi.fn();
