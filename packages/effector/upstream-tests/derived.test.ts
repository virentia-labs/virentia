...
// Virentia upstream skip reason: Проверяет exact diagnostics/warnings для derived units и внутренних targetability соглашений Effector; публичная совместимость покрывается is.targetable и операторами фасада.
describe.skip("basic checks for that the derived untis in target are forbidden", () => {
  ...
  // Virentia upstream skip reason: Проверяет скрытое поле unit.targetable; внешний API для этого — is.targetable.
  test.skip("targetable field works", () => {
...
// Virentia upstream skip reason: Проверяет exact diagnostics/warnings для derived units и внутренних targetability соглашений Effector; публичная совместимость покрывается is.targetable и операторами фасада.
describe.skip("call of derived events", () => {
...
// Virentia upstream skip reason: Проверяет exact diagnostics/warnings для derived units и внутренних targetability соглашений Effector; публичная совместимость покрывается is.targetable и операторами фасада.
describe.skip(".on/.reset with derived stores", () => {
