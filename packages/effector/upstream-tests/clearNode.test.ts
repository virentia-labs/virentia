...
// Virentia upstream skip reason: Низкоуровневый lifecycle графа Effector: clearNode, createNode, withRegion, step и graphite. В Virentia нет совместимой graphite-модели удаления узлов.
describe.skip("upstream clearNode.test.ts", () => {
  muteErrors("forward");
  ...
  describe("supports sample in withRegion", () => {
    ...
    test("units will not die after clearNode call", () => {
      const fnTrigger = vi.fn();
      const fnSource = vi.fn();
      const fnTarget = vi.fn();
  ...
  describe.each([{ regionWrap: false }, { regionWrap: true }])(
    ...
    ({ regionWrap }) => {
      ...
      let sampleFn: Mock<(...args: [number]) => number>;
      let externalTriggerFn: Mock<(...args: any) => any>;
      let externalTargetFn: Mock<(...args: any) => any>;
      let regionalUnitFn: Mock<(...args: any) => any>;
      let region: Node;
      ...
      beforeEach(() => {
        sampleFn = vi.fn((x) => x);
        externalTriggerFn = vi.fn();
        externalTargetFn = vi.fn();
        regionalUnitFn = vi.fn();
      ...
      describe("regional unit in external sample clock", () => {
        ...
        test("last unit in clock array is regional (should destroy the link)", () => {
          ...
          expect({
            ...
            externalTargetFn: EXTERNAL_UNIT_CAN_KILL ? [0, 1, 4, 7] : [0, 1, 2, 4, 7],
          });
        });
      ...
      test("regional unit in external sample filter (should destroy the link)", () => {
        ...
        expect({
          ...
          externalTargetFn: KILL_EXTERNAL && SAMPLE_FILTER_SUPPORTED ? [0, 2] : [0, 1, 2],
        });
      });
      ...
      test("regional .on support", () => {
        const handlerFn = vi.fn((x: number) => x);
      ...
      describe("external unit should survive", () => {
        let handlerFn: Mock<(...args: [number]) => number>;
        beforeEach(() => {
          handlerFn = vi.fn((x) => x);
        });
        test("store .map", () => {
