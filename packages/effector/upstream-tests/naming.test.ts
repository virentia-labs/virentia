...
import { createEvent, createStore, createDomain, combine, sample } from "@virentia/effector";
import { debug } from "patronum";
import { unitObjectName } from "../naming";
import { argumentHistory } from "effector/fixtures";

// Virentia upstream skip reason: SID/compositeName/domain naming semantics пока не совпадают с Effector.
describe.skip("upstream naming.test.ts", () => {
  ...
  describe("naming", () => {
    ...
    test("unnamed object store", () => {
      ...
      expect(app.compositeName?.fullName).toBe("combine(combine(firstName, lastName))");
  ...
  describe("sample support", () => {
    let fn: MockInstance;
    let fnGroup: MockInstance;
    beforeEach(() => {
      fn = vi.spyOn(console, "info").mockImplementation(() => {});
      fnGroup = vi.spyOn(console, "groupCollapsed").mockImplementation(() => {});
    });
    afterEach(() => {
      fn.mockRestore();
      fnGroup.mockRestore();
