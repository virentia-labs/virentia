...
// Virentia upstream skip reason: Проверяет точный eager ordering между effect handler, watch и finally в Effector; публичный attach lifecycle покрыт локальным facade-тестом.
it.skip("map params and results by provided functions", async () => {
  const fn = vi.fn();
...
// Virentia upstream skip reason: Проверяет точный eager ordering attach/source effect lifecycle Effector; Virentia сохраняет события lifecycle, но не обещает тот же промежуточный порядок.
it.skip("support source store", async () => {
  const fn = vi.fn();
  const requestFX = createEffect({
...
// Virentia upstream skip reason: Проверяет точный eager ordering attach/source-shape lifecycle Effector; внешний контракт attach source покрыт локальными тестами.
it.skip("support source shape", async () => {
  const fn = vi.fn();
  const requestFX = createEffect({
    handler(data: { url: string; token: string }) {
      fn({ tag: "effect handler", data });
...
// Virentia upstream skip reason: Закрепляет Effector priority semantics для watch, sample target array и attached effect source reads; Virentia lazy scheduler не обещает такой промежуточный порядок.
test.skip("interaction with watch and parallel updates", async () => {
  const fn = vi.fn();
...
// Virentia upstream skip reason: Закрепляет Effector batch/priority semantics для cascaded sample target array; текущий compatibility слой покрывает одиночный attach/source runtime contract.
it.skip("should read actual store value", () => {
  const fn = vi.fn();
