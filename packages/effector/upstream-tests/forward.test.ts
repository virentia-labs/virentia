...
import { forward, createEvent, createStore, createNode } from "@virentia/effector";
import { muteErrors } from "effector/fixtures";

describe("upstream forward.test.ts", () => {
  muteErrors("forward");
  ...
  it("should forward data from one event to another", () => {
    ...
    expect(fn.mock.calls).toEqual([["should been forwarded [1]"], ["should been forwarded [2]"]]);
  });

  // Virentia upstream skip reason: raw createNode wiring is a low-level graphite compatibility check, not the deprecated public forward runtime contract.
  describe.skip("raw nodes support", () => {
    test("single ones", () => {
  ...
  // Virentia upstream skip reason: exact Effector validation messages include loc/debug metadata that Virentia does not emulate for deprecated forward.
  it.skip("should validate arguments", () => {
    expect(() => {
      //@ts-expect-error
      forward({});
    }).toThrowErrorMatchingInlineSnapshot(
      `"[forward] (/src/effector/__tests__/forward.test.ts:163:4): expect \\"from\\" to be a unit (store, event or effect) or array of units"`,
    );
    expect(() => {
      //@ts-expect-error
      forward({ from: createStore(null) });
    }).toThrowErrorMatchingInlineSnapshot(
      `"[forward] (/src/effector/__tests__/forward.test.ts:169:4): expect \\"to\\" to be a unit (store, event or effect) or array of units"`,
    );
    expect(() => {
      //@ts-expect-error
      forward({ to: createStore(null) });
    }).toThrowErrorMatchingInlineSnapshot(
      `"[forward] (/src/effector/__tests__/forward.test.ts:175:4): expect \\"from\\" to be a unit (store, event or effect) or array of units"`,
    );
  });
});
