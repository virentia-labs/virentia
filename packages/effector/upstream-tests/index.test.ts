...
describe("upstream index.test.ts", () => {
  // Virentia upstream skip reason: graphite/showstep snapshots assert Effector internal graph shape.
  test.skip("graphite", () => {
    const fn = vi.fn();
    const fn1 = vi.fn();
  ...
  // Virentia upstream skip reason: showcase includes graphite/showstep snapshots; non-graph runtime basics are covered below.
  test.skip("showcase", () => {
    const fn = vi.fn();
  ...
  // Virentia upstream skip reason: @@observable interop is not implemented in the facade yet.
  describe.skip("symbol-observable support", () => {
