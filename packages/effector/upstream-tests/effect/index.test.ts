...
describe("execution order", () => {
  // Virentia upstream skip reason: Закрепляет точный eager ordering Effector между handler/watch/done/finally/pending; Virentia сохраняет lifecycle, но порядок отличается из-за scheduler-а.
  it.skip("should run watchers and promise resolvers in order", async () => {
    const fn = vi.fn();
    const fx = createEffect({
      handler() {
        fn("handler");
  ...
  // Virentia upstream skip reason: Закрепляет точный eager ordering Effector для async effect lifecycle; внешний контракт lifecycle покрыт соседними passing-тестами.
  it.skip("should run watchers and promise resolvers in order (async)", async () => {
    const fn = vi.fn();
    const fx = createEffect({
      async handler() {
        fn("handler");
  ...
  it("handle sync effect watchers in correct order", async () => {
    ...
    await eff("run");
    expect(argumentHistory(fn)).toEqual(["run", { params: "run", result: [1, 2, 3] }]);
  });
...
// Virentia upstream skip reason: Ошибка завязана на Babel-plugin inferred name `fooFx`; сама runtime validation .use остается в facade.
it.skip("should validate .use argument", () => {
  expect(() => {
    //@ts-expect-error
...
describe("@farfetched/core agreement", () => {
  ...
  test("should allow patching of effect handler", async () => {
    ...
    const runnerNode = ((fx as any as { graphite: Node }).graphite.scope as { runner: Node })
      .runner;
