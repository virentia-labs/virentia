...
import { createEffect } from "@virentia/effector";
import { argumentHistory } from "effector/fixtures";

// Virentia upstream skip reason: Патчит скрытый effect.create hook Effector; это внутренний DI escape hatch, а не внешний API.
describe.skip("upstream effect/di.test.ts", () => {
  test("effect.create single argument", async () => {
    ...
    //@ts-expect-error
    effect.create = vi.fn((payload, args) => oldCreate(payload, args));
    const baz = vi.fn();
