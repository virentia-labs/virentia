...
import { createEffect, createDomain } from "@virentia/effector";
import { muteErrors } from "effector/fixtures";

describe("upstream effect/name.test.ts", () => {
  muteErrors("getType");
  ...
  test("should return it's own name on effect.getType()", () => {
    ...
    expect(createEffect({ handler: () => "done!", name: "foo" }).getType()).toBe("foo");
    //@ts-expect-error
    expect(createEffect({ handler: () => "done!" }, { name: "foo" }).getType()).toBe("foo");
    expect(
      createEffect(
