...
// Virentia upstream skip reason: Лезет в graphite Node/family/meta/loc, чтобы проверить debug location internals Effector.
describe.skip("loc support", () => {
  ...
  function findJointLoc(targetNode: Node) {
    if (targetNode.family.links.length === 0) throw Error(`this node didn't used in sample`);
    let jointNode: Node;
...
describe("temporal consistency", () => {
  ...
  // Virentia upstream skip reason: Backward-clocking relies on Effector pure-phase priority; Virentia lazy graph does not promise this intermediate ordering.
  test.skip("clock triggers earlier than source during same pure phase", () => {
    const fn = vi.fn();
    const trigger = createEvent<number>();
...
// Virentia upstream skip reason: Проверяет exact Effector loc-prefixed diagnostic; facade сохраняет runtime validation без upstream loc строки.
it.skip("should not accept undefined clocks", () => {
  expect(() => {
    sample({
      //@ts-expect-error
...
describe("sample type", () => {
  ...
  test.each`
    ...
    ${createEvent()}  | ${createStore(0)} | ${"event"}
    ${createEvent()}  | ${createEvent()}  | ${"event"}
  `(`$kind <- $source.kind by $clock.kind with handler`, ({ source, clock, kind }) => {
    expect(
      //@ts-expect-error
...
describe("sample", () => {
  ...
  describe("sample with event as source", () => {
    ...
    `(
      ...
      ({ batch, resultDirect, resultBacktracking }) => {
        ...
        // Virentia upstream skip reason: `batch=true` backtracking depends on Effector eager priority. `batch=false` and direct event source behavior remain covered.
        test.skipIf(batch)("backtracking", () => {
          const fn = vi.fn();
          const A = createEvent<number>();
          const B = A.map((x) => ({ x }));
    ...
    // Virentia upstream skip reason: Sibling event backtracking fixes exact Effector ordering between mapped/filterMap branches; Virentia lazy graph keeps stable final values but not this intermediate priority.
    it.skip("works with sibling events", () => {
      const fn1 = vi.fn();
      const fn2 = vi.fn();
      const A = createEvent<number>();
      const B = A.map((b) => ({ b }));
    ...
    test("edge case: no updates until first source update " + "even when clock is store", () => {
      const fn = vi.fn();
      const data = createEvent<any>();
  ...
  describe("sample with effect as source", () => {
    // Virentia upstream skip reason: Fire-and-forget effect-as-source before awaiting effect start depends on Effector started-event priority; awaited watcher behavior is covered below.
    test.skip("effect", () => {
      const fn = vi.fn();
    ...
    describe("event call will not break watchers", () => {
      it.each`
        ...
        ${true}
        ${false}
      `("event call will not break watchers (batch = $batch)", async ({ batch }) => {
        const fn1 = vi.fn();
...
describe("event/effect sampling behavior (issue #633)", () => {
  // Virentia upstream skip reason: Проверяет Effector priority с target array и pending-gate в одном тике; Virentia lazy scheduler допускает оба запуска.
  test.skip("event behavior", () => {
    const fn = vi.fn();
  ...
  // Virentia upstream skip reason: Проверяет Effector priority с effect target array и pending-gate в одном тике; Virentia lazy scheduler допускает оба запуска.
  test.skip("effect behavior", () => {
    const fn = vi.fn();
...
describe("greedy deprecation", () => {
  ...
  // Virentia upstream skip reason: Проверяет exact Effector loc-prefixed warning; facade предупреждает без upstream loc строки.
  test.skip("greedy true is deprecated", () => {
    const clock = createEvent();
    const target = createEvent();
    sample({
      clock,
  ...
  // Virentia upstream skip reason: Проверяет exact Effector loc-prefixed warning; facade предупреждает без upstream loc строки.
  test.skip("greedy false is deprecated", () => {
    const clock = createEvent();
    const target = createEvent();
    sample({
      clock,
