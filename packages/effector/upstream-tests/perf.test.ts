...
// Virentia upstream skip reason: Stress/perf сценарий для внутренней топологии Effector, не функциональный compatibility test.
describe.skip("upstream perf.test.ts", () => {
  test("perf", async () => {
    const stores = [] as any[];
    let factoryACalls = 0;
    factoryC();
  ...
  async function runTestCase({
    ...
    /** typical is 50-70ms */
    console.log(`${name}: avg ${avg(filtered).toFixed(2)} median ${median(filtered).toFixed(2)}`);
    ...
    function percentile(list: number[], avg: number, percentile: number) {
      const z = list.map((n, i) => [Math.abs(n - avg), i]).sort((a, b) => a[0] - b[0]);
