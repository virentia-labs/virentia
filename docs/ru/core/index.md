...
Стор хранит значение. Событие сообщает о факте или доменном намерении. Эффект запускает работу, которая завершится позже. Реакция связывает эти части в поведение.

Если хочется добавить метод вроде `setCount`, лучше назвать событие языком задачи: `incremented`, `reset`, `open`, `submitted`, `messageReceived`. Так модель говорит о смысле действия, а не о техническом способе изменить поле.
